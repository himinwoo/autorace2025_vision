...
class SlideWindow:
    ...
    def __init__(self):
        self.x_previous = 320  # 640 → 320 (1/2 비율)



    def slidewindow(self, img, lane_mode=0):

        x_location = 320  # 640 → 320 (1/2 비율)
        ...
        # lane_mode에 따라 차선 감지 영역 강제 설정
        # 0: 양쪽 모두, 1: 왼쪽만, 2: 오른쪽만
        if lane_mode == 1:  # 왼쪽 차선만 감지
            good_right_inds = []  # 오른쪽 차선 무시
        elif lane_mode == 2:  # 오른쪽 차선만 감지
            good_left_inds = []  # 왼쪽 차선 무시
