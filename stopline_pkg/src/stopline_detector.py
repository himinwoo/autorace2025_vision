...
class StopLineDetector:
    ...
    """
    def __init__(self, down_hist_start_line=400, stopline_threshold=10, histogram_threshold=800,
        ...
        Note:
            count_threshold는 detect() 메서드 호출 시 동적으로 전달됩니다.
        """
    ...
    def detect(self, bin_img, count_threshold=15):
        """
        이진화된 이미지에서 정지선을 감지합니다.
        
        Args:
            bin_img: 이진화된 이미지 (numpy array)
            count_threshold: 정지선 감지 확정을 위한 연속 감지 횟수 (state별로 다름)
        ...
        if len(stopline_indices) > 0:
            ...
            if self.stopline_threshold < stopline_diff:
                ...
                if self.stopline_count >= count_threshold:
                    detected = True
            else:
                # 점진적 감소
